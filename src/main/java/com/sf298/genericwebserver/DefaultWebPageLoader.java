 ...
 * @author saud
 */
public class DefaultWebPageLoader {

	public static void addContextsToServer(SHTMLServer server, UserManager um, ParamEditorV pe, DevicesManager dm) {
		addStaticContexts(server, um);
		addServices(server, um);
		addDynamicPages(server, um, dm);
		DefaultWebPageLoader.addRedirect(server, "/", "/login.html");
	}

	private static void addStaticContexts(SHTMLServer server, UserManager um) {
		File root = new File(DefaultWebPageLoader.class.getClassLoader().getResource("WebPages").getFile().replace("%20", " "));
